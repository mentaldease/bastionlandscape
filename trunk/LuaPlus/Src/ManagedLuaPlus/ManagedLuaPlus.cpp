...
public:
	...
	String* ToWString()
	{
		try
		{
			return new String(m_object->ToWString());
		}
		catch (const LuaPlus::LuaException& e)
		{
			throw new LuaException(new String(e.GetErrorMessage()));
	...
	String* GetWString()
	{
		try
		{
			return new String(m_object->GetWString());
		}
		catch (const LuaPlus::LuaException& e)
		{
			throw new LuaException(new String(e.GetErrorMessage()));
...
*/
	...
	**/
	...
	String* GetWString()				{  return new String(m_object->GetWString());  }
...
public:
	...
	void PushNil()								{  m_state->PushNil();    }
	void PushInteger(int n)		    			{  m_state->PushInteger(n);  }
	void PushNumber(double n)					{  m_state->PushNumber((lua_Number)n);  }
	void PushLString(String* s, size_t len)		{  m_state->PushLString(MarshalStringANSI(s), len);  }
	void PushLWString(String* s, size_t len)	{  m_state->PushLWString(MarshalStringUnicode(s), len);  }
	void PushString(String* s)					{  m_state->PushString(MarshalStringANSI(s));  }
	void PushWString(String* s)					{  m_state->PushWString(MarshalStringUnicode(s));  }
...
//	void PushCFunction(LuaStateCFunction f)	{  m_state->pushcclosure((CFunction)f, 0);  }
	void PushBoolean(bool value)				{  m_state->PushBoolean(value);  }
	void PushLightUserData(void* p)				{  m_state->PushLightUserData(p);  }
	...
	LuaStackObject* NewTable(int /*size*/)		{  return new LuaStackObject(this, m_state->NewTable());  }
	void* NewUserData(size_t size)				{  return m_state->NewUserData(size);  }
	...
	LuaStackObject* GetGlobal_Stack(String* name)
	{
		LuaPlus::LuaStackObject obj = m_state->GetGlobal_Stack(MarshalStringANSI(name));
		return new LuaStackObject(this, obj);
	}
	...
	int Ref(int lock)							{  return m_state->Ref(lock);  }
	void Unref(int ref)							{  m_state->Unref(ref);  }
...
//	int DoWString(const lua_WChar* str, const char* name);
	int LoadBuffer(String* buff, size_t size, String* name)
	{
		return m_state->LoadBuffer(MarshalStringANSI(buff), size, MarshalStringANSI(name));
	}
...
//	int DoWBuffer(const lua_WChar* buff, size_t size, const char *name);
	...
	int CoResume(int arg)						{  return m_state->CoResume(arg);  }

	// Garbage-collection functions
	int GetGCThreshold()						{  return m_state->GetGCThreshold();  }
	int GetGCCount()							{  return m_state->GetGCCount();  }
	void SetGCThreshold(int newthreshold)		{  m_state->SetGCThreshold(newthreshold);  }

	// Miscellaneous functions
	int Error()									{  return m_state->Error();  }

	int Next(int index)							{  return m_state->Next(index);  }
	int GetN(int index)							{  return m_state->GetN(index);  }
	...
	void CollectGarbage()						{  m_state->SetGCThreshold(0);  }
	...
	// Arg functions.
	int TypeError(int narg, const char* tname)							{  return m_state->TypeError(narg, tname);  }
	int ArgError(int narg, const char* extramsg)						{  return m_state->ArgError(narg, extramsg);  }
	...
	void ArgCheck(bool condition, int numarg, const char* extramsg)		{  m_state->ArgCheck(condition, numarg, extramsg);  }
	const char* CheckString(int numArg)									{  return m_state->CheckString(numArg);  }
	const char* OptString(int numArg, const char* def)					{  return m_state->OptString(numArg, def);  }
	...
	// Debug functions.
/*	int GetStack(int level, Debug* ar)			{  return m_state->getstack(level, ar);  }
	int GetInfo(const char* what, Debug* ar)		{  return m_state->getinfo(what, ar);  }
	const char* GetLocal(const Debug* ar, int n)	{  return m_state->getlocal(ar, n);  }
	const char* SetLocal(const Debug* ar, int n)	{  return m_state->setlocal(ar, n);  }

	int SetHook(Hook func, unsigned long mask)	{  return m_state->sethook(func, mask);  }
	Hook GetHook()									{  return m_state->gethook(m_state);  }
	unsigned long GetHookMask()							{  return m_state->gethookmask(m_state);  }
*/
...
{
	m_state = LuaPlus::LuaState::Create(false);
	m_ownState = true;
}
...
{
	return new LuaStackObject(GetState(), m_object->PushStack());
}
