...
namespace LuaInterface
{
	...
	class CheckType
	{
		...
		internal ExtractValue checkType(IntPtr luaState,int stackPos,Type paramType) 
		{
			...
			{
				if(LuaDLL.lua_type(luaState,stackPos)==LuaTypes.LUA_TUSERDATA)
					return extractUserdata;
			}
			else if(paramType.Equals(typeof(LuaFunction))) 
			{
				if(LuaDLL.lua_type(luaState,stackPos)==LuaTypes.LUA_TFUNCTION)
					return extractFunction;
			}
			else if(typeof(Delegate).IsAssignableFrom(paramType) && 
				LuaDLL.lua_type(luaState,stackPos)==LuaTypes.LUA_TFUNCTION) 
			{
				return new ExtractValue(new DelegateGenerator(translator,paramType).extractGenerated);
			}
			else if(paramType.IsInterface &&
				LuaDLL.lua_type(luaState,stackPos)==LuaTypes.LUA_TTABLE)
			{
